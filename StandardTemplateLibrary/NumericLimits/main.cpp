...
template <typename T>
bool almost_equal(const T x, const T y) //especially for floats
{
...
int main()
{
    ...
    const auto d1 = 0.2;    // == 1 / 5
    const auto d2 = 1.0 / (std::sqrt(5.0) * std::sqrt(5.0)); // == 1 / 5

    std::cout << d1 << '\n';
    std::cout << d2 << '\n';

    //We can set it so that the first 17 decimal places are displayed and should not be rounded up
